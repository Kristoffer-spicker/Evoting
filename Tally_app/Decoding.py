...
def decode_value(key, value):
    if key == "spk":
       ...
       return bytes.fromhex(value)
        ...
        return [
            ECC.EccPoint(int(value[0]['x']), int(value[0]['y']), 'P-192'),
            ECC.EccPoint(int(value[1]['x']), int(value[1]['y']), 'P-192'),
            gmpy2.mpz(value[2])
        ]
    elif key in proofs:
...
def decode_ciphertext(c):
    ...
    if isinstance(c, dict):
        ...
        return [
            ECC.EccPoint(int(c['c1']['x']), int(c['c1']['y']), 'P-192'),
            ECC.EccPoint(int(c['c2']['x']), int(c['c2']['y']), 'P-192'),
            gmpy2.mpz(c['r_anti'])
        ]
    elif isinstance(c, list):
        # List format: [{x,y}, {x,y}, int]
        return [
            ECC.EccPoint(int(c[0]['x']), int(c[0]['y']), 'P-192'),
            ECC.EccPoint(int(c[1]['x']), int(c[1]['y']), 'P-192'),
            gmpy2.mpz(c[2])
        ]
    else:
